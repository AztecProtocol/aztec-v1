...
contract('DividendCalc', (accounts) => {
    ...
    describe('failure states', () => {
        ...
        it('validate failure for residual commitment message that does not satisfy proof relation', async () => {
            ...
            const { proofData, challenge } = aztec.proof.dividendComputation.constructProof(wrongRelationship, za, zb, accounts[0]);
        ...
        it('validates failure for zero residual note value', async () => {
            ...
            const { proofData, challenge } = aztec.proof.dividendComputation.constructProof(zeroResidualValue, za, zb, accounts[0]);
        ...
        it('validates failure for fake trusted setup key', async () => {
            ...
            const fakeTrustedSetupKey = [web3Utils.randomHex(32), web3Utils.randomHex(32), web3Utils.randomHex(32), web3Utils.randomHex(32)];

            await exceptions.catchRevert(dividendCalcContract.validateDividendCalc(proofData, challenge, za, zb, fakeTrustedSetupKey, {
                from: accounts[0],
                gas: 4000000,
            }));
        });
        ...
        it('validates failure for z_a, z_b > kMax', async () => {
            ...
            const { proofData, challenge } = aztec.proof.dividendComputation.constructProof(testNotes, zaHigh, zbHigh, accounts[0]);
