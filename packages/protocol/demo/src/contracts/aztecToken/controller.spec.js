    ...
    describe('success states', async function success() {
        ...
        beforeEach(async () => {
            ...
            await transactionsController.getTransactionReceipt(
                await erc20Controller.mint(wallets[0].address, wallets[1].address, SCALING_FACTOR.mul(new BN(10000)).toString(10))
            );
            await transactionsController.getTransactionReceipt(
                await erc20Controller.mint(wallets[0].address, wallets[0].address, SCALING_FACTOR.mul(new BN(10000)).toString(10))
            );
        });
        ...
        it('can issue a series of confidentialTransfer transactions', async () => {
            ...
            await transactionsController.getTransactionReceipt(
                await erc20Controller.approve(
                    wallets[0].address,
                    aztecTokenAddress,
                    SCALING_FACTOR.mul(new BN(10000)).toString(10)
                )
            );
            const inputNotes = [
