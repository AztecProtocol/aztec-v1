...
contract('AZTEC - ERC20 Token Bridge Tests', (accounts) => {
    describe('success states', () => {
        ...
        it('successfully blinds 100,000 tokens into 5 zero-knowledge notes', async () => {
            ...
            const { proofData, challenge } = joinSplit.constructProof(commitments, m, accounts[0], kPublic);
        ...
        it('successfully enacts a join split transaction, splitting a 10000, 13000 notes into a 3000, 20000 notes', async () => {
            ...
            const m = 2;
            const { proofData, challenge } = joinSplit.constructProof(commitments, m, accounts[0], 0);
        ...
        it('successfully enacts a join split transaction, redeeming 11999 tokens', async () => {
            ...
            const m = 2;
            const kPublic = 11999;
            const { proofData, challenge } = joinSplit.constructProof(commitments, m, accounts[3], kPublic);
            const { address } = aztecToken;
            const signatures = [
    ...
    describe('failure states', () => {
        ...
        it('invalid signatures cannot be used to spend non-existant notes', async () => {
            ...
            const { proofData, challenge } = joinSplit.constructProof(commitments, 1, accounts[3], 100);
        ...
        it('cannot create note with no owner', async () => {
            ...
            initialCommitments = commitments;
            const kPublic = GROUP_MODULUS.sub(tokensTransferred);
            const { proofData, challenge } = joinSplit.constructProof(commitments, m, accounts[4], kPublic);
        ...
        it('validate failure if AZTEC note owner attempts withdrawl greater than note value', async () => {
            ...
            const kPublic = 9001;
            const { proofData, challenge } = joinSplit.constructProof(commitment, m, accounts[0], kPublic);
            const { address } = aztecToken;
            const signatures = [
