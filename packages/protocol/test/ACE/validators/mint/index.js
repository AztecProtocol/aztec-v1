...
const Keccak = keccak;
const { mint, proofUtils, joinSplit } = proof;
const { outputCoder, inputCoder, encoderFactory } = abiEncoder;
...
contract('AdjustSupply Tests for Mint Proofs', (accounts) => {
    ...
    describe('Failure States', () => {
        ...
        it('should fail if zero notes provided', async () => {
            ...
            const { proofData: proofDataRaw, challenge } = mint.constructProof([], senderAddress);
        ...
        it('should fail if points NOT on curve', async () => {
            ...
            const proofData = inputCoder.mint(proofDataRaw, challenge, inputOwners, outputOwners, []);
        ...
        it('should fail if scalars are zero', async () => {
            ...
            const proofData = inputCoder.mint(scalarZeroProofData, challenge, inputOwners, outputOwners, outputNotes);
        ...
        it('should fail if number of notes supplied is less than the minimum (2) i.e. no output notes', async () => {
            ...
            const { proofData: proofDataRaw, challenge } = mint.constructProof([newTotalMinted], senderAddress);
