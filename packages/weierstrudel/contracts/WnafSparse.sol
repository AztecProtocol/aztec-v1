...
// WAIT
// if we mask off w to get m
// then we can just add m into w and then mask
// if the 5th bit is high then that will overflow and add into 6th bit
// and the other bits don't mean diddly, right?
// and we'll need to mask off m if we want to use our little 'set high bit' trick because we can't have the 7th bit set to true
// dup1 0x1f and dup1 dup4 mstore8 <- m w o
// add <mask> and = 9 ops. ho ho ho

// vs
// dup1 dup3 mstore8 dup1 0x10 and add add <mask> and = 10 ops

/*
 wnaf_start:
            dup1 dup1 callvalue sub
            and
            swap2
            269
            dup4
