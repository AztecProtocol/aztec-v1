...
#define ENDO_WNAF_FIXED_IMPL = takes(2) returns(1) {
    SET_WNAF_TABLE()
    ENDO_WNAF_FIXED()
}

#define ENDO_WNAF_FIXED_B_IMPL = takes(2) returns(1) {
    SET_WNAF_TABLE()
    ENDO_WNAF_FIXED_B()
}

#define WNAF_SLICE_A_IMPL = takes(2) returns(1) {
    SET_WNAF_TABLE()
    WNAF_SLICE_A()
}
#define WNAF_SLICE_B_IMPL = takes(2) returns(1) {
    SET_WNAF_TABLE()
    WNAF_SLICE_B()
}
`;
...
describe.only('sparse wnaf', function describe() {
    ...
    it('macro ENDO_WNAF_FIXED correctly calculates a w=5 NAF of a 127-bit number, with added memory offset', async () => {
        ...
        const baseOffset = 1025;
        const { memory } = await wnaf('ENDO_WNAF_FIXED_IMPL', [new BN(baseOffset), endoScalar], [], []);
        for (let i = 0; i < reference.length; i += 1) {
            const memoryOffset = baseOffset + (i * 32);
            if (reference[i] && reference[i].gt(new BN(0))) {
                expect(reference[i].eq(new BN(memory[memoryOffset], 16).umod(thirtyTwo))).to.equal(true);
    ...
    it('macro WNAF_SLICE_A correctly calclates a wnaf slice', async () => {
        ...
        const { memory, stack } = await wnaf('WNAF_SLICE_A_IMPL', [w, new BN(baseOffset)], [], []);
        expect(new BN(memory[832], 16).umod(thirtyTwo).toString(10)).to.equal('23');
        expect(stack.length).to.equal(2);
        expect(stack[0].toString(10)).to.equal('832');
        expect(stack[1].toString(2)).to.equal('111011110100000');
    });

    it('macro WNAF_SLICE_B correctly calclates a wnaf slice', async () => {
        const w = new BN('1110111100101110', 2);
        const baseOffset = 800;
        const { memory, stack } = await wnaf('WNAF_SLICE_B_IMPL', [new BN(baseOffset), w], [], []);
        expect(new BN(memory[832], 16).umod(thirtyTwo).toString(10)).to.equal('23');
        expect(stack.length).to.equal(2);
        expect(stack[1].toString(10)).to.equal('832');
        expect(stack[0].toString(2)).to.equal('111011110100000');
    });

    it('macro ENDO_WNAF_FIXED_B correctly calculates a w=5 NAF of a 127-bit number, with added memory offset', async () => {
        const scalar = bn128Reference.randomScalar();
        let { k1: endoScalar } = endomorphism.endoSplit(scalar);
        const reference = referenceWnaf.wnaf(endoScalar);
        endoScalar = endoScalar.or(new BN('1000000000000000000000000000000000', 16));
        const baseOffset = 1025;
        const { memory } = await wnaf('ENDO_WNAF_FIXED_B_IMPL', [endoScalar, new BN(baseOffset)], [], []);
        for (let i = 0; i < reference.length; i += 1) {
            const memoryOffset = baseOffset + (i * 32);
            if (reference[i] && reference[i].gt(new BN(0))) {
                expect(reference[i].eq(new BN(memory[memoryOffset], 16).umod(thirtyTwo))).to.equal(true);
