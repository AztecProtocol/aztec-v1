...
import { zkAssetApis, zkNoteApis, userApis } from '../config/apis';
...
class LiveDocUpdate extends Component {
    ...
    parseSource = async () => {
        ...
        const apiType = Object.keys(apiTypeMapping).find((type) => apiTypeMapping[type].indexOf(name) >= 0);
        if (!apiType) return;

        const urlPath = process.env.NODE_ENV === 'development' ? '../../public' : '';
        ...
        const parsedArguments = APItags.tags.filter(
            ({ tag }) => tag !== 'returns' && tag !== 'function' && tag !== 'description',
        );
    ...
    render() {
        ...
        return (
            <Block padding="l 0">
                {!!(parsedDescription && parsedDescription.description) && <MethodDescription {...parsedDescription} />}
                {<MethodExplanation />}
