...
import { FlexBox, Block, Text, Loader, Icon, Button } from '@aztec/guacamole-ui';
import { AZTEC_API_KEY, REQUIRED_NETWORK } from '../../config/aztec';
...
class SdkPlaygroundControls extends PureComponent {
    ...
    handleProfileChanged = (type, value, error) => {
        const { loggingIn: wasLoggingIn } = this.state;
        const nextState = {
            aztecAccount: null,
        ...
        switch (type) {
            case 'accountChanged': {
                ...
                const { account: prevAccount, autoLogin } = this.state;
                if (autoLogin) {
            ...
            case 'aztecAccountChanged': {
                const { account, network } = window.aztec.web3;
        ...
        this.setState(nextState, nextAction);
    };
    ...
    async doGetTestEth() {
        const { setupLogingInIframe } = this.props;
    ...
    async doGetTestERC20() {
        const { zkAssetAddress, setupLogingInIframe } = this.props;
    ...
    async refreshAsset() {
        const { zkAssetAddress } = this.props;
    ...
    async refreshBalances() {
        const { account, asset } = this.state;

        const weiBalance = !account ? 0 : await window.aztec.web3.eth.getBalance(account.address);
        const ethBalance = parseFloat(fromWei(weiBalance)).toFixed(2);

        const linkedTokenBalance = !asset || !asset.valid ? 0 : await asset.balanceOfLinkedToken();
    ...
    renderLoginButton(message) {
        return <Button className={styles.button} theme="primary" text={message} size="m" onClick={this.login} expand />;
    }

    renderMessage(message) {
        // eslint-disable-line class-methods-use-this
        return <Text text={message} size="xxs" />;
    }

    renderLoader(message) {
        // eslint-disable-line class-methods-use-this
        return (
            <FlexBox className="lh0" valign="center">
                <Block padding="0 xs">
                    <Loader theme="white" size="xxs" />
                </Block>
                {!!message && (
                    <Block padding="0 xs">
    ...
    renderControls() {
        const { sdkLoaded, loggingIn, aztecAccount, prevAccount, network, error } = this.state;
        ...
        if (loggingIn) {
            const loadingMessage = prevAccount ? 'Switching AZTEC account...' : 'Logging in to AZTEC account...';
        ...
        const { onRunCode, isRunning } = this.props;
        const { ethBalance, linkedTokenBalance, refreshingBalances, loadingTestEth, loadingTestERC20 } = this.state;

        return (
            <FlexBox align="space-between" stretch expand>
                <FlexBox align="flex-start" valign="center">
                    <Block padding="0 l">
                        <Text text={refreshingBalances ? 'Updating ETH' : `${ethBalance} ETH`} color="white-lighter" size="xs" />
                    </Block>
                    <Button
                ...
                <Block padding="xs m" hasBorderBottom>
                    <FlexBox align="space-between">
                        <FlexBox aling="flex-start">
                            <Text text="Ethereum Address:" size="xs" />
                            <Block left="s">
                                <Text text={(account && account.address) || ''} size="xs" color="grey" />
                            </Block>
                        </FlexBox>
                        <Text text={(network && network.name) || ''} size="xs" color="orange" />
                    </FlexBox>
                </Block>
                {children}
                <Block className={styles.controls} background="grey-darker">
                    {this.renderControls()}
                </Block>
            </div>
        );
