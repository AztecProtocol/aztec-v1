...
export default async function getTestERC20(zkAssetAddress, iframeLog, amount = 100) {
    ...
    if (!zkAssetAddress) {
        iframeLog.error(
            new Error(
                'Cant mint tokens. ZkAsset Address is not defined /n Make sure you assign a value to the variable "const zkAssetAddress" in the editor.',
            ),
        );
        return;
    }
