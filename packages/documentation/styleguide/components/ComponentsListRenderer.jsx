...
import { spacingMap, colorMap, roundedCornerMap, fontSizeMap, fontWeightMap } from '../../src/styles/guacamole-vars';
        ...
        <div className={classes.root}>
            {visibleItems.map(({ name, heading, href, visibleName: defaultVisibleName, content, external, initialOpen }) => {
                ...
                const linkNode = isStatic ? (
                    <div onClick={handleClick}>{displayName}</div>
                ) : (
                    <Link href={href} target={external ? '_blank' : undefined} onClick={handleClick}>
                        {displayName}
                    </Link>
                );
