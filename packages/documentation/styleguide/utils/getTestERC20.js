...
const mintERC20 = async (zkAssetAddress, iframeLog) => {
    ...
    if (!zkAssetAddress) {
        iframeLog.error(
            new Error(
                'Cant mint tokens. Zk Asset Address is not defined /n Make sure you call name the variable "const zkAssetAddress" in the editor',
            ),
        );
        return;
    }
    ...
    const { account } = web3Service;
    web3Service.registerInterface(IERC20, {
        name: 'ERC20Mintable',
    });
    try {
