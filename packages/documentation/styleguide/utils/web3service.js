...
class Web3Service {
    ...
    async init({ providerUrl = '', contractsConfig, account } = {}) {
        this.reset();
    ...
    registerContract(config, { name = '', address = '' } = {}) {
        if (!this.web3) {
            return null;
        }
        ...
        if (!config.abi) {
            console.error(`Contract object "${contractName}" doesn't have an abi.`);
            return null;
        }
        ...
        const contract = new this.web3.eth.Contract(config.abi, contractAddress);
    ...
    registerContractsConfig(contractsConfig) {
        contractsConfig.forEach(({ name, config, address }) => {
            if (!address) {
    ...
    getAddress(contractName) {
        const { address } = this.contract(contractName) || {};
        return address;
    }
    ...
    async deploy(config, constructorArguments = []) {
        const { contractName, abi, bytecode } = config;
    ...
    triggerMethod = async (type, { method, contractAddress, fromAddress, args }) => {
        const methodSetting =
            (args.length &&
                ...
                args[args.length - 1]) ||
            null;
        const methodArgs = methodSetting ? args.slice(0, args.length - 1) : args;
        ...
        if (type === 'sendSigned') {
            if (!contractAddress) {
                console.log('contractAddress should be passed');
                return null;
            }
            ...
            const { privateKey } = methodSetting || {};
    ...
    useContract(contractName, contractAddress = null) {
        return {
            method: (methodName) => {
                ...
                return {
                    call: async (...args) =>
                        this.triggerMethod('call', {
                            method,
                            fromAddress: this.account.address,
                            args,
                        }),
                    send: async (...args) =>
                        this.triggerMethod('send', {
                            method,
                            fromAddress: this.account.address,
                            args,
                        }),
                    sendSigned: async (...args) =>
                        this.triggerMethod('sendSigned', {
                            method,
                    ...
                    useGSN: (gsnConfig) => {
                        if (!gsnConfig) {
                            console.log('Cannot use gsn as "gsnConfig" was not defined');
                        }
            ...
            events: (eventName) => {
                ...
                return {
                    where: async ({ filter = {}, fromBlock = 1, toBlock = 'latest' }) =>
                    ...
                    subscribe: (options = { filter: {}, fromBlock: 1 }, callback) =>
                    ...
                    subscribeAll: (options = { filter: {}, fromBlock: 1 }, callback) =>
            ...
            at: (address) => {
                ...
                if (!this.abis[contractName]) {
                    console.warn(`'${contractName}' is not registered as an interface.`);
                }
