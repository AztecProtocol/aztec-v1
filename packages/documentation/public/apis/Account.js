import { note as noteUtils } from 'aztec.js';
import uniq from 'lodash/uniq';
import Web3Service from '~/client/services/Web3Service';
import ConnectionService from '~/client/services/ConnectionService';
import ApiError from '~/client/utils/ApiError';

const dataProperties = ['address', 'linkedPublicKey', 'spendingPublicKey'];
...
class Account {
    ...
    async createNote(value, userAccess = []) {
        ...
        return noteUtils.create(this.spendingPublicKey, value, noteAccess, this.address);
    }
    ...
    async encryptMessage(message) {
        ...
        const { encrypted } =
            (await ConnectionService.query('encryptMessage', {
    ...
    async decryptMessage(message) {
        ...
        const { decrypted } =
            (await ConnectionService.query('decryptMessage', {
                address: this.address,
                message,
            })) || {};
