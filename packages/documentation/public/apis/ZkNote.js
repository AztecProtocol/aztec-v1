import { fromViewingKey } from '~/utils/note';
import ConnectionService from '~/client/services/ConnectionService';
import provePrivateRange from '~/client/apis/privateRange/prove';

const dataProperties = ['noteHash', 'value', 'viewingKey', 'owner', 'asset', 'status'];
...
export default class ZkNote {
    ...
    async export() {
        ...
        const { note } = (await ConnectionService.query('noteWithViewingKey', { id: this.id })) || {};

        if (!note || !note.decryptedViewingKey) {
            return null;
        }

        const { decryptedViewingKey, owner = {} } = note;
    ...
    async grantAccess(addresses) {
        ...
        const { success } =
            (await ConnectionService.query('grantNoteAccess', {
                id: this.id,
     ...
     *
     */
    async equal(comparisonNote, { sender = '', remainderNote = null } = {}) {
        if (!this.visible) {
            return false;
        }
     ...
     *
     */
    async greaterThan(comparisonNote, { sender = '', remainderNote = null } = {}) {
        if (!this.visible) {
            return false;
        }
     ...
     *
     */
    async lessThan(comparisonNote, { sender = '', remainderNote = null } = {}) {
        if (!this.visible) {
            return false;
        }
     ...
     *
     */
    async greaterThanOrEqualTo(comparisonNote, { sender = '', remainderNote = null } = {}) {
        if (!this.visible) {
            return false;
        }
     ...
     *
     */
    async lessThanOrEqualTo(comparisonNote, { sender = '', remainderNote = null } = {}) {
        if (!this.visible) {
            return false;
        }
