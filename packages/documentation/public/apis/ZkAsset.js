...
import { tokenToNoteValue, noteToTokenValue, recoverJoinSplitProof } from '~/utils/transformData';
...
const dataProperties = ['address', 'linkedTokenAddress', 'scalingFactor', 'canAdjustSupply', 'canConvert', 'token'];
...
export default class ZkAsset {
     ...
     */
    async balance() {
        const { balance } = (await ConnectionService.query('assetBalance', { id: this.id })) || {};
    ...
    async totalSupplyOfLinkedToken() {
        ...
        let totalSupply = 0;
        try {
            totalSupply = await Web3Service.useContract('ERC20')
    ...
    balanceOfLinkedToken = async (account) => {
        ...
        if (!accountAddress) {
            ({ address: accountAddress } = Web3Service.account);
        }

        try {
            balance = await Web3Service.useContract('ERC20')
                .at(this.linkedTokenAddress)
                .method('balanceOf')
                .call(accountAddress);
        } catch (error) {
    ...
    allowanceOfLinkedToken = async (owner = '', spender = '') => {
        ...
        if (!ownerAddress) {
            ({ address: ownerAddress } = Web3Service.account);
        }
        ...
        try {
            allowance = await Web3Service.useContract('ERC20')
                .at(this.linkedTokenAddress)
                .method('allowance')
                .call(ownerAddress, spenderAddress);
        } catch (error) {
    ...
    subscribeToBalance = async (subscriber) => {
        ...
        return this.subscriptions.add('ASSET_BALANCE', this.id, subscriber);
    };

    unsubscribeToBalance = async (subscriber) => this.subscriptions.remove('ASSET_BALANCE', this.id, subscriber);
     ...
     *
     */
    deposit = async (transactions, { numberOfOutputNotes, userAccess = [], returnProof, sender, publicOwner } = {}) => {
        if (!this.linkedTokenAddress) {
            throw new ApiError('zkAsset.private', {
                fn: 'deposit',
            });
        ...
        const { success, outputNotes, proofData } = await ConnectionService.query('constructProof', {
        ...
        let proof;
        if (proofData) {
            proof = proofData ? await recoverJoinSplitProof(proofData) : null;
        }
     ...
     *
     */
    withdraw = async (amount, { to, numberOfInputNotes, inputNoteHashes, returnProof, sender } = {}) => {
        if (!this.linkedTokenAddress) {
            throw new ApiError('zkAsset.private', {
                fn: 'withdraw',
            });
        ...
        const { success, proofData } = await ConnectionService.query('constructProof', {
    ...
    send = async (
        transactions,
        { numberOfInputNotes, numberOfOutputNotes, inputNoteHashes, userAccess, returnProof, sender, publicOwner } = {},
    ) => {
        const { success, outputNotes, proofData } = await ConnectionService.query('constructProof', {
     ...
     *
     */
    createNoteFromBalance = async (amount, { userAccess = [], numberOfInputNotes, numberOfOutputNotes = 1 } = {}) => {
        const { notes } =
            (await ConnectionService.query('constructProof', {
     ...
     *
     */
    fetchNotesFromBalance = async ({ greaterThan, lessThan, equalTo, numberOfNotes } = {}) =>
        ConnectionService.query('fetchNotesFromBalance', {
            assetAddress: this.address,
