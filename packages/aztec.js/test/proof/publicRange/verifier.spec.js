...
describe('Public range proof verifier', () => {
    describe('Success States', () => {
        it('should construct a valid dividend computation proof', async () => {
            ...
            const { valid } = publicRange.verifier.verifyProof(proofData, challenge, sender, kPublic);
    ...
    describe('Failure States', () => {
        it('should REJECT for unsatisfied proof relations', async () => {
            ...
            const { proofData, challenge } = publicRange.constructProof(wrongRelationship, kPublic, sender);

            const { valid, errors } = publicRange.verifier.verifyProof(proofData, challenge, sender, kPublic);
        ...
        it('should REJECT for fake challenge', async () => {
            ...
            const { proofData } = publicRange.constructProof(testNotes, kPublic, sender);
            ...
            const { valid, errors } = publicRange.verifier.verifyProof(proofData, fakeChallenge, sender, kPublic);
            expect(valid).to.equal(false);
            expect(errors.length).to.equal(1);
            expect(errors[0]).to.equal(errorTypes.CHALLENGE_RESPONSE_FAIL);
            parseInputs.restore();
        ...
        it('should REJECT for fake proof data', async () => {
            ...
            const { challenge } = publicRange.constructProof(testNotes, kPublic, sender);

            const fakeProofData = [...Array(2)].map(() =>
                [...Array(6)].map(() => `0x${padLeft(crypto.randomBytes(32).toString('hex'), 64)}`),
            );

            const { valid, errors } = publicRange.verifier.verifyProof(fakeProofData, challenge, sender, kPublic);
            expect(valid).to.equal(false);
