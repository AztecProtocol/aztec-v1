...
describe('note tests', () => {
    describe('success state', () => {
        ...
        it('correctly encodes the metadata of a set of notes', () => {
            ...
            expect(new BN(sharedSecrets[0].slice(2), 16).umod(GROUP_MODULUS).eq(noteArray[0].a.fromRed())).to.equal(
                true,
            );
            expect(new BN(sharedSecrets[1].slice(2), 16).umod(GROUP_MODULUS).eq(noteArray[1].a.fromRed())).to.equal(
                true,
            );
            expect(new BN(sharedSecrets[2].slice(2), 16).umod(GROUP_MODULUS).eq(noteArray[2].a.fromRed())).to.equal(
                true,
            );
            expect(new BN(sharedSecrets[3].slice(2), 16).umod(GROUP_MODULUS).eq(noteArray[3].a.fromRed())).to.equal(
                true,
            );
        });

        it('produces k, a values of 0 for a note created from a note public key', () => {
    ...
    describe('failure states', () => {
        it('throws if given a public key and a viewing key', () => {
            const note = notes.create(secp256k1.generateAccount().publicKey, 100);
        ...
        it('throws if given a non-string public key', () => {
            let message = '';
            try {
        ...
        it('throws if given an incorrect length public key', () => {
            const note = notes.create(secp256k1.generateAccount().publicKey, 100);
        ...
        it('throws if given a non-string viewing key', () => {
            let message = '';
            try {
        ...
        it('throws if given an incorrect length viewing key', () => {
            const note = notes.create(secp256k1.generateAccount().publicKey, 100);
