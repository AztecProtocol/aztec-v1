...
describe('abiEncoder.outputCoder tests', () => {
    ...
    it('outputCoder can encode output note', () => {
        const encoded = new HexString(bilateralSwap.outputCoder.encodeOutputNote(notes[0]));
    ...
    it('outputCoder can encode input note', () => {
        const encoded = new HexString(bilateralSwap.outputCoder.encodeInputNote(notes[0]));
        expect(isHex(encoded)).to.equal(true);
    ...
    it('outputCoder can encode notes', () => {
        ...
        const encoded = new HexString(bilateralSwap.outputCoder.encodeNotes(inputNotes, true));
        ...
        for (let i = 0; i < length; i += 1) {
            ...
            sum += (byteLength + 0x40);
            expect(encodedNote).to.equal(bilateralSwap.outputCoder.encodeOutputNote(inputNotes[i]));
        }
    ...
    it('outputCoder can encode a proof output', () => {
        ...
        const publicValue = 0;
        const encoded = new HexString(bilateralSwap.outputCoder.encodeProofOutput({
            inputNotes,
            outputNotes,
            publicOwner,
            publicValue,
        ...
        expect(String(encodedInputNotes)).to.equal(bilateralSwap.outputCoder.encodeNotes(inputNotes, false));
        expect(String(encodedOutputNotes)).to.equal(bilateralSwap.outputCoder.encodeNotes(outputNotes, true));
    ...
    it('outputCoder can encode proof outputs', () => {
        ...
        const encoded = new HexString(bilateralSwap.outputCoder.encodeProofOutputs(proofs).slice(2));
        expect(isHex(encoded)).to.equal(true);
        ...
        for (let i = 0; i < numProofs; i += 1) {
            ...
            expect(proofOutput).to.equal(bilateralSwap.outputCoder.encodeProofOutput(proofs[i]).toLowerCase());
            sum += (byteLength + 0x40);
        }
    ...
    it('outputCoder can decode an encoded output note', () => {
        const encoded = bilateralSwap.outputCoder.encodeOutputNote(notes[0]);
        const result = bilateralSwap.outputCoder.decodeOutputNote(encoded);
    ...
    it('outputCoder can decode an encoded input note', () => {
        const encoded = bilateralSwap.outputCoder.encodeInputNote(notes[0]);
        const result = bilateralSwap.outputCoder.decodeInputNote(encoded);
        expect(result.gamma.eq(notes[0].gamma)).to.equal(true);
        expect(result.sigma.eq(notes[0].sigma)).to.equal(true);
        expect(result.owner).to.equal(notes[0].owner);
        expect(result.noteHash).to.equal(notes[0].noteHash);
    ...
    it('outputCoder can decode encoded input notes', () => {
        const encoded = bilateralSwap.outputCoder.encodeNotes([notes[0], notes[1]], true);
        const result = bilateralSwap.outputCoder.decodeNotes(encoded, true);
    ...
    it('outputCoder can decode a proof output', () => {
        const encoded = bilateralSwap.outputCoder.encodeProofOutput({
            inputNotes: [notes[0], notes[1]],
            outputNotes: [notes[2], notes[3]],
            publicOwner: padLeft('0', 64),
            publicValue: new BN(0),
        });
        const result = bilateralSwap.outputCoder.decodeProofOutput(encoded);
    ...
    it('outputCoder can decode proof outputs', () => {
        ...
        const encoded = bilateralSwap.outputCoder.encodeProofOutputs(proofOutputs);
        const result = bilateralSwap.outputCoder.decodeProofOutputs(encoded);
