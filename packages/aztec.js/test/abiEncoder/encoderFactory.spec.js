...
describe('inputCoder tests', () => {
    ...
    describe('bilateralSwap encoderFactory tests', () => {
        it('bilateralSwap is correctly formatted', () => {
            ...
            const { proofData, challenge } = bilateralProof.constructProof(
            ...
            const result = new HexString(
                abiEncoder.inputCoder
                    .bilateralSwap(proofData, challenge, inputOwners, outputOwners, outputNotes)
                    .slice(2),
            );
            ...
            const recoveredProofData = new HexString(
                result.slice(offsetToProofData, offsetToProofData + numNotes * 0xc0),
            );
            for (let i = 0; i < numNotes; i += 1) {
            ...
            const recoveredInputOwners = new HexString(
                result.slice(offsetToInputOwners, offsetToInputOwners + 2 * 0x20),
            );
            ...
            const recoveredOutputOwners = new HexString(
                result.slice(offsetToOutputOwners, offsetToOutputOwners + 2 * 0x20),
            );
            expect(recoveredOutputOwners.slice(0x00, 0x20)).to.equal(
                padLeft(outputOwners[0].slice(2).toLowerCase(), 64),
            );
            expect(recoveredOutputOwners.slice(0x20, 0x40)).to.equal(
                padLeft(outputOwners[1].slice(2).toLowerCase(), 64),
            );
    ...
    describe('joinSplit tests work', () => {
        it('joinSplit is correctly formatted', () => {
            ...
            // Main
            const { proofData, challenge } = joinSplitProof.constructProof(
                [...inputNotes, ...outputNotes],
                m,
                senderAddress,
                0,
            ...
            const result = new HexString(
                abiEncoder.inputCoder
                    .joinSplit(proofData, m, challenge, publicOwner, inputSignatures, outputOwners, outputNotes)
                    .slice(2),
            );
