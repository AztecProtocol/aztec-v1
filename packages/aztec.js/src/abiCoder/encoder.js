 ...
 * characters each represent 0.5 bytes
 */
encoder.encodeMetadata = (metadata) => {
 ...
 * characters each represent 0.5 bytes
 */
encoder.encodeProofData = (proofData) => {
    const { length } = proofData;
    const noteString = proofData.map((notes) => encoder.encodeNotes(notes));
