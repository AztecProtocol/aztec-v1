...
const { BN128_GROUP_REDUCTION, errorTypes } = devUtils.constants;
...
dividendComputation.constructBlindingFactors = (notes, zaBN, zbBN, rollingHash) => {
    const bkArray = [];
    let x = new BN(0).toRed(BN128_GROUP_REDUCTION);
    x = rollingHash.keccak(BN128_GROUP_REDUCTION);
    ...
    return notes.map((note, i) => {
        ...
        if (i === 2) {
            // residual note
            const zbRed = zbBN.toRed(BN128_GROUP_REDUCTION);
            const zaRed = zaBN.toRed(BN128_GROUP_REDUCTION);
