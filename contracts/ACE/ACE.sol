...
contract Validator {
    function validate(bytes proofData, address sender, bytes32[6] crs) public view returns (
...
contract Extractor {
    function extractProofData(bytes memory proofData) internal view returns (
    ...
    function extractNoteData(bytes memory note) internal view returns (
        bytes32 noteHash,
        address owner,
...
contract ACE {
    ...
    constructor (bytes32[6] _commonReferenceString, address[] _initialProofs, uint16[] _initialProofTypes) {
        owner = msg.sender;
    ...
    function validateProof(uint16 _proofType, bytes _proofData, address _sender) external returns (
